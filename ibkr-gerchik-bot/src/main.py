...
def run_job(job_name: str) -> Dict[str, object]:
    ensure_directories()
    state_path = SETTINGS.paths.runtime_dir / "state.json"
    state = _load_state(state_path)
    alerter = SlackAlerter()

    if job_name == "weekly":
        metrics = run_weekly(state.get("weekly_results", []))
        return {"job": job_name, "metrics": metrics}

    broker = IBKRClient()
    broker.connect()
    market_data = MarketDataService(broker)
    order_manager = OrderManager(broker, alerter)
    ...
    if kill_switch and job_name in {"open", "intraday"}:
        ...
        return {"job": job_name, "blocked": True, "reasons": reasons}

    if job_name == "premarket":
        watchlist = run_premarket(market_data)
        state["watchlist"] = watchlist
        _save_state(state_path, state)
        return {"job": job_name, "watchlist_count": len(watchlist)}
    ...
    if job_name == "open":
        ...
        _save_state(state_path, state)
        return {"job": job_name, "executed": executed}

    if job_name == "intraday":
        actions = run_intraday(broker, alerter, state.get("tracked_positions", []))
        return {"job": job_name, "actions": actions}
    ...
    if job_name == "eod":
        ...
        return {"job": job_name, "summary": summary}
